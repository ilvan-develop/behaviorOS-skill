...
description: Governança cross-cutting do FinPay. Use quando verificar regras que afetam múltiplas skills, validar anti-patterns transversais, ou consultar o single source of truth para tenant isolation, audit trail, design tokens, secrets, PCI-DSS, e quality gates. Carregar antes de qualquer tarefa que envolva mais de um domínio.
metadata:
  stack: governance, behavioros
  scope: cross-cutting
  version: "1.0.0"
---

# Enterprise Governance - FinPay

## 1. Visão Geral

Esta skill é o **single source of truth** para regras que afetam múltiplas skills do FinPay. Cada skill técnica mantém os seus anti-patterns locais (domain-specific), mas regras transversais são centralizadas aqui.
...
- Configurar monorepo (usar skill `monorepo-architecture-guardian`)
...
| 11 | Consultar docs atualizadas | INSTRUCTIONS.md §11 |
| 12 | Carregar skill relevante | INSTRUCTIONS.md §12 |
| 13 | Anti-patterns são proibidos | INSTRUCTIONS.md §13 |
| 14 | Design system first | INSTRUCTIONS.md §14 |
| 15 | Anti-patterns cross-cutting | INSTRUCTIONS.md §15 |
...
```typescript
// MAU - Service sem organizationId
async createPayment(data: CreatePaymentDto) {
  return this.prisma.payment.create({ data });
}

// BOM - Service com organizationId
async createPayment(data: CreatePaymentDto, organizationId: string) {
...
```markdown
### [Nome da Regra]
> **Regra central:** Ver `enterprise-governance` secção "AG-[XX] [Nome]".
...
echo "=== Fitness Functions - FinPay ==="

# 1. Verificar PrismaClient isolado
echo "[1/5] Verificar PrismaClient..."
...
echo "[2/5] Verificar importações shadcn..."
...
echo "[3/5] Verificar dependências circulares..."
...
echo "[4/5] Verificar deriva de dependências..."
...
# 5. Verificar FLOAT para dinheiro
echo "[5/5] Verificar Float para dinheiro..."
BAD=$(grep -r "amount Float\|price Float\|total Float" packages/*/prisma/ apps/*/prisma/ --include="*.prisma" -l 2>/dev/null || true)
if [ -n "$BAD" ]; then
  echo "ERRO: Float usado para dinheiro (usar Decimal): $BAD"
  exit 1
fi
echo "  OK"
...
- `references/duplication-map.md` — Mapa de 14 duplicações resolvidas
