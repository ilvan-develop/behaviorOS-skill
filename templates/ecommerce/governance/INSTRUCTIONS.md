...
### 3. Tenant isolation
- `organizationId` obrigatório em toda query (se multi-tenant)
...
/agent_loop --scope F3-CART    → Usa scope específico
```
...
Fase 0: Fundação (DEVE terminar primeiro)
    ...
    └──→ F1: Auth + Users
              │
              ├──→ F2: Catálogo (pode paralelo com F3)
              │
              └──→ F3: Carrinho + Checkout (pode paralelo com F2)
                        │
                        └──→ F4: Gestão (depende de F3)
                                  │
...
```json
{
  "scopeId": "{{PROJECT_NAME}}-F3-CART",
  "phase": "F3",
  "name": "Carrinho + Checkout",
  "description": "Implementar carrinho e fluxo de checkout",
  "deliverables": [
    "API de Carrinho",
    "API de Checkout",
...
```json
{
  ...
  "agent": "backend",
  "action": "write",
  "target": "src/cart/cart.service.ts",
  "gate": "scope",
  "result": "pass",
  "autonomyLevel": "L1",
  "scopeId": "{{PROJECT_NAME}}-F3-CART"
