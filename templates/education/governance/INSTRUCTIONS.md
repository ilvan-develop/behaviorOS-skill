...
/agent_loop --phase F3         → Usa fase F3 como scope
/agent_loop --scope F3-CONTENT → Usa scope específico
```
...
### Conclusão de Fase
- **Fases não-críticas** (F0, F1, F2, F5): Avança automaticamente quando gates passam
- **Fases críticas** (F3, F4): Para e pede aprovação humana
...
Fase 0: Fundação (DEVE terminar primeiro)
    ...
    └──→ F1: Auth + Users
              │
              ├──→ F2: Cursos (pode paralelo com F3)
              │
              └──→ F3: Conteúdo (pode paralelo com F2)
                        │
                        └──→ F4: Progresso (depende de F2 e F3)
                                  │
...
```json
{
  "scopeId": "{{PROJECT_NAME}}-F3-CONTENT",
  "phase": "F3",
  "name": "Conteúdo",
  "description": "Implementar gestão de conteúdo educacional",
  "deliverables": [
    "API de Conteúdo",
    "Frontend de Conteúdo",
    "Upload de Materiais",
    "Testes E2E"
  ],
  "constraints": {
    "immutableRules": [1, 2, 3, 4, 5, 6, 7, 8],
    "compliance": ["LGPD", "WCAG"],
    "stack": ["NestJS", "oRPC", "Prisma"],
    "architecture": ["contracts-first", "audit-trail", "accessibility"]
  },
  "allowedAgents": [
    "architect",
    "backend",
...
```json
{
  ...
  "agent": "frontend",
  "action": "write",
  "target": "src/components/CourseCard.tsx",
  "gate": "scope",
  "result": "pass",
  "autonomyLevel": "L1",
  "scopeId": "{{PROJECT_NAME}}-F3-CONTENT"
...
6. **NUNCA** ignorar requisitos de acessibilidade
7. **NUNCA** executar comandos destrutivos (DROP, TRUNCATE, mass deletions) sem gate
8. **NUNCA** executar qualquer tarefa sem pelo menos uma Skill validada carregada
9. **NUNCA** permitir que um agente modifique suas próprias permissões ou o Governance Kernel
