...
- Registros de acesso a dados de saúde são obrigatórios

### 6. LGPD
- Proteção de dados pessoais desde o schema
- Consentimento obrigatório para coleta de dados
- Direito ao esquecimento implementado
- Dados de saúde exigem consentimento explícito

### 7. Criptografia de dados sensíveis
- Dados de saúde criptografados em repouso e trânsito
- Chaves de criptografia gerenciadas externamente
- Nunca armazenar chaves junto com os dados
...
/agent_loop --scope F3-CHARTS  → Usa scope específico
```
...
### Conclusão de Fase
- **Fases não-críticas** (F0, F1, F2): Avança automaticamente quando gates passam
- **Fases críticas** (F3, F4, F5): Para e pede aprovação humana
...
Fase 0: Fundação (DEVE terminar primeiro)
    ...
    └──→ F1: Auth + Users
              │
              ├──→ F2: Pacientes (pode paralelo com F3)
              │
              └──→ F3: Prontuários (pode paralelo com F2)
                        │
                        └──→ F4: Agendamentos (depende de F2 e F3)
                                  │
...
```json
{
  "scopeId": "{{PROJECT_NAME}}-F3-CHARTS",
  "phase": "F3",
  "name": "Prontuários",
  "description": "Implementar prontuários eletrônicos",
  "deliverables": [
    "API de Prontuários",
    "Frontend de Prontuários",
  ...
  "constraints": {
    ...
    "architecture": ["contracts-first", "tenant-isolation", "audit-trail", "encryption"]
  },
  "allowedAgents": [
    "architect",
    "backend",
...
```json
{
  ...
  "agent": "backend",
  "action": "write",
  "target": "src/chart/chart.service.ts",
  "gate": "scope",
  "result": "pass",
  "autonomyLevel": "L1",
  "scopeId": "{{PROJECT_NAME}}-F3-CHARTS"
...
4. **NUNCA** executar operações reais em dev/test
5. **NUNCA** contornar auth, autorização, RBAC
6. **NUNCA** armazenar dados de saúde em texto plano
