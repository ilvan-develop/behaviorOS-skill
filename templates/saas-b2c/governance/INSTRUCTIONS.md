...
/agent_loop --scope F2-FEATURE → Usa scope específico
```
...
### Conclusão de Fase
- **Fases não-críticas** (F0, F1, F4): Avança automaticamente quando gates passam
- **Fases críticas** (F2, F3): Para e pede aprovação humana
...
Fase 0: Fundação (DEVE terminar primeiro)
    ...
    └──→ F1: Auth + Users
              │
              ├──→ F2: Core Feature (depende de F1)
              │
              └──→ F3: Billing (depende de F1)
                        │
                        └──→ F4: Endurecimento (depende de todas)
```
...
```json
{
  ...
  "phase": "F2",
  "name": "Core Feature",
  "description": "Implementar feature principal do SaaS",
  "deliverables": [
    "API Principal",
    "Frontend Principal",
    "Testes E2E"
